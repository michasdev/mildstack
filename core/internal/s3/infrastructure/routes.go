...
func Routes() []orchestrator.Route {
	routes := make([]orchestrator.Route, 0, len(bucketRoutes())+len(bucketSubresourceRoutes())+len(bucketEventRoutes())+len(versioningRoutes())+len(objectLockRoutes())+len(objectRoutes())+len(multipartRoutes()))
