...
func TestRenderStatusUsesStructuredTheme(t *testing.T) {
	...
	if got, want := RenderStatus(DefaultTheme(), presenter), "Runtime Status\nState: ready\n\nServices\n  alpha v1\n  beta v2\n\nPorts\n  8080\n  9090\n"; got != want {
...
func TestRenderEmptyStatusAndPlainPorts(t *testing.T) {
	...
	if got, want := RenderPorts(DefaultTheme(), presenter), ""; got != want {
		t.Fatalf("unexpected empty ports render:\n got %q\nwant %q", got, want)
	}
	if got, want := RenderReadiness(DefaultTheme(), presenter), "State: not_ready"; got != want {
