...
func TestDefaultRootIncludesS3AndRegistersRoutesThroughSharedRegistrar(t *testing.T) {
	t.Helper()

	root := DefaultRoot()
	if got, want := len(root.Services), 1; got != want {
	...
	var hook orchestrator.StateHook = &stateHookStub{}
	...
	if got, want := state["service"], "s3"; got != want {
		t.Fatalf("unexpected service state: got %v want %v", got, want)
	}
}
